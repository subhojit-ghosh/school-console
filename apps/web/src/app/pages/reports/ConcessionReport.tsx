...
export default function ConcessionReport() {
  ...
  const [sortStatus, setSortStatus] = useState<DataTableSortStatus>({
    columnAccessor: 'totalConcession',
    direction: 'desc',
  });
  ...
  const downloadExcel = async () => {
    try {
      ...
      const { data } = await httpClient.get(
        endpoints.reports.concessionsExport(),
        {
          params,
          responseType: 'blob',
        }
      ...
      link.download = `Concession Report - ${
        parts.join(' - ') || 'All'
      }.xlsx`;
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            render: (row: any) => (
              <Currency value={row.totalConcession || 0} />
            ),
            sortable: true,
          },
          {
